...
describe('QuickView Component', () => {
  ...
  it('should apply all custom props', () => {
     ...
     * horizontalCenter
     */
    const testStyle2 = TestHelper.getStyle(target, { horizontalCenter: true }, type);
    expect(testStyle2.alignItems).toBe('center');
    expect(testStyle2.justifyContent).not.toBe('center');

    const testStyle3 = TestHelper.getStyle(target, { horizontalCenter: true, row: true }, type);
     ...
     * verticalCenter
     */
    const testStyle4 = TestHelper.getStyle(target, { verticalCenter: true }, type);
    expect(testStyle4.alignItems).not.toBe('center');
    expect(testStyle4.justifyContent).toBe('center');

    const testStyle5 = TestHelper.getStyle(target, { verticalCenter: true, row: true }, type);
    ...
    const props1 = TestHelper.getComponent(target, { onPress }, type);
    expect(props1.name()).toBe('ForwardRef');

    const props2 = TestHelper.getComponent(target, {}, type);
